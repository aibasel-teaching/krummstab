...
does not matter.
2. Adding a `submission.json` file: Add a `submission.json` file to the folder 
created in the previous step. The internal structure of this file needs 
particular attributes:
   - `team`: list of students of the team, where each student entry is a list
   of the form `[ "first_name", "last_name", "email@stud.unibas.ch" ]`
   - `adam_id`: a string representing the ID generated by ADAM for the
   submission
...
3. Creating a feedback folder: Create a new folder with the name `feedback` in
the directory created in the first step. When you have marked the team, you can
add your feedback files here.
4. Modifying the `points.json` file: Add the team key with the points that the
team gets to the `points.json` file. The team key consists of the ADAM ID and
the alphabetically sorted last names of all team members in the following
format: `ID_Last-Name1_Last-Name2`
